...
class ERC20 {
  ...
  symbol: string;
  decimal: number;
  approve_amount?: BigNumber;

  private contract: Contract;

  constructor(address: string, provider: Signer | Provider, symbol: string, decimal = 18, approve_amount?: BigNumber) {
  ...
  async displayedTotalSupply(): Promise<string> {
    ...
    return parseFloat(formatUnits(supply, this.decimal)).toFixed(0);
  }
}
