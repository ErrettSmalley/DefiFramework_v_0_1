...
describe("Vault", async () => {
    ...
    it("Vault.func => setMaxLeverage", async () => {
        ...
        await timelock.setUsdmAmounts(v.address,[dai.address], [parseEther("10000")]);
    ...
    it("Vault.func => setVaultUtils", async () => {
        ...
        await v.setUsdmAmount(dai.address,0);
    });
    ...
    it("Vault.func => allWhitelistedTokensLength", async () => {
        ...
        // console.log(`v.getUtilisation: ${await v.getUtilisation(dai.address)}`);
        console.log(`v.getNextFundingRate: ${await v.getNextFundingRate(dai.address)}`);
        // console.log(`v.getGlobalShortDelta: ${await v.getGlobalShortDelta(dai.address)}`);
        ...
        // console.log(`v.getRedemptionCollateral: ${await v.getRedemptionCollateral(dai.address)}`);

        // console.log(`v.getRedemptionCollateralUsd: ${await v.getRedemptionCollateralUsd(dai.address)}`);

        console.log(`v.getRedemptionAmount: ${await v.getRedemptionAmount(dai.address, parseEther("1.2345"))}`);
    });

    it("Vault.func => getTargetUsdmAmount", async () => {
        await timelock.signalSetGov(v.address, owner.address);
        await forwardTime(500000);
        await timelock.setGov(v.address,owner.address);

        console.log(`v.getTargetUsdmAmount: ${await v.getTargetUsdmAmount(dai.address)}`);
