...
const func: DeployFunction = async function ({deployments, getNamedAccounts, network, getChainId}) {
    ...
    console.log(`vaultReader.address: ${vaultReader.address}`);

    console.log(">> MLP setting...");
    await execute("MLP", {from: owner}, "setMinter", MlpManager.address, true);
    await execute("RewardRouter", {from: owner}, "initialize",
        WNative.address, GMX.address, AddressZero, AddressZero, MLP.address,
        AddressZero, AddressZero, AddressZero, AddressZero, AddressZero,
        MlpManager.address, AddressZero, AddressZero
    );
